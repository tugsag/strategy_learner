...
class DTLearner(object):
    ...
    def build_tree(self, dataX, dataY):
        ...
        if np.all(dataY == dataY[0]):
            ...
            best_feat = self.get_split_feat(dataX, dataY)
    ...
    @staticmethod
    def get_split_feat(currX, currY):
        corr_list = []
        for i in range(currX.shape[1]):
            corr_list.append(np.abs(np.corrcoef(currX[:, i], currY)[1][0]))

        return corr_list.index(max(corr_list))
